...
%El titulo creo que es muy largo y confunde la primera parte.
%Propongo: A Novel Method Based
%on Fuzzy Inference Systems, Multi-agent Systems and Genetic Programming
%for the Forecast of Financial Markets
...
\IEEEcompsocthanksitem Mario Garc{\'{\i}}a Valdez is with Affiliation not available. }}% <-this % stops a space
...
There are many tools that can be used to forecast the next values of a time-series, and many others specialized in forecasting the more complex financial time-series.  % ¿A que te refieres con tools? techniques? 

%Prefiero que empieces así:
Since many decades ago, financial traders have implemented statistical models to forecast time-series, in a field called Technical Analysis. %Referencia 
%and a field called Technical Analysis was created. % Da alguna referencia, que importancia tiene ese campo de estudio en tu trabajo?
This type of market analysis is characterized by using past financial data to describe 
certain aspects of a market, such as its volatility, trend, and momentum.% Referencia 
These tools receive the name of Technical Indicators (TI). % No digas tools así nada más, que es tools? indicators? variables? etc. 
Some examples of TI are those created by Welles Wilder, such as Relative Strength Index (RSI) and Average Directional Index \cite{wilder1978new}, and an example of an oscillator (a type of TI which usually tells if a market is overbought or oversold) is the Stochastic Oscillator \cite{schirding1984stochastic}, created by George Lane. %Comenta algo más de estos indicadores, ¿por que los mencionas tan temprano?
...
%Di algo aqui sobre la explicación que sigue, o prueba quitandola.
%A technique that can be used in Fundamental Analysis is to perform a group selection of companies, and analyze their economic behaviors to establish a general forecast for each of them. After performing this analysis, one can narrow the group of companies and decide on a proper trading strategy to follow according to the arrived conclusions. To analyze a company, one can resort to examine its business plan and its management.

A current approach to perform these analyses is to use Machine Learning (ML) techniques. %referencia.
%With ML, one can use inputs provided by Technical or Fundamental Indicators and create regression models.
%Muy poco formal. Mejor algo como:
Using ML algorithms, researchers create regression models using Technical or Fundamental Indicators as trainig datasets. %ref
...
%En el párrafo anterior se debe recalcar el porque y lo novedoso del método.
%La oración: 
...
%Debe segir de una explicación de lo novedoso de el método. Esto está en el párrafo anterior pero está encerrado en mucha paja. Trata de simplificarlo.
%Por ejemplo como inicio:
This work proposes a method based on ABM to construct a Decision Support System (DSS) that aids traders on the process of trading financial markets. 
...
The method uses a combination of Fuzzy Inference Systems (FIS), Genetic Programming (GP) \cite{poli2008field} \cite{Koza1992} and Multi-agent Systems (MAS) \cite{Shoham2009} to produce models that can explain the behavior of complex systems such as that of a financial market. %Por qué? que ventajas de que manera es novedoso?
Agents in the proposed method are selfish agents that don't intervene with other agents, but there's a higher order process that is dictating how they must organize. In this case, the collective work of every agent must give a ``zero-sum'': the sum of their forces must be equal to the observed price, or in other words, they must adjust to the real prices. To achieve this, each agent uses a FIS to act intelligently, and its Membershif Functions (MF) are found using GP. Specifically, the predicates and consequents of the FIS are generated using sum of sines that are found using a modified algorithm based on GP. The modified algorithm implements communities of agents that perform different operations among their agents: crossover, migration, replace and mutation. In the end, the developed algorithm gives as result the community of agents that obtained the absolute value of the lowest Mean-Squared Error (MSE) plus the sum of its agents scores (explained in Section \ref{proposed-method}). At first, the authors of this work had the hypothesis that the developed system would enable the user to examine the MF of each agents' FIS to develop new economic theories, but, even though the system achieved remarkable results, the generated MF are too chaotic to be interpreted with ease.
%Creo que el parrafo anterior tiene menos paja, y se entiende mejor lo de la Intro.
%Faltaría por ejemplo poner lo de la comunidad de agentes y otras cosillas, siempre siendo muy claro.
...
%El parrafo anterior no se ma hace bueno, casi dices hay mucho pero como hay que mencionar algunos pues va.
%Creo que deberías empezar con los primeros trabajos del tema, o traerte mejor el parrafo de la sección anterior, aqui lo pego: solo faltaria una mini intro.
...
% Empieza con el Método, no los Datasets, es más creo que esta sección debería de ir en Experimentos.
% Empieza con lo de community of agents.
\subsection{Dataset}
\label{dataset}
...
\begin{center}
\includegraphics[width=1\columnwidth]{figures/mfs-lisp-example/mfs-lisp-example.png}
\end{center}
\end{figure}
...
\begin{center}
\includegraphics[width=0.7\columnwidth]{figures/flowchart-without/flowchart-without.png}
\end{center}
\end{figure}
...
\begin{center}
\includegraphics[width=0.7\columnwidth]{figures/flowchart-with/flowchart-with.png}
\end{center}
\end{figure}
...
\begin{center}
\includegraphics[width=0.7\columnwidth]{figures/fis-vs-no-fis/fis-vs-no-fis.png}
\end{center}
\end{figure}
...
\begin{center}
\includegraphics[width=1\columnwidth]{figures/avg-rate-of-return/avg-rate-of-return.png}
\end{center}
\end{figure}
...
\begin{center}
\includegraphics[width=1\columnwidth]{figures/rates-of-return-heatmap/rates-of-return-heatmap.png}
\end{center}
\end{figure}
...
\begin{center}
\includegraphics[width=1\columnwidth]{figures/rates-of-return-aa-heatmap/rates-of-return-aa-heatmap.png}
\end{center}
\end{figure}
...
\begin{center}
\includegraphics[width=1\columnwidth]{figures/rates-of-return-csco-heatmap/rates-of-return-csco-heatmap.png}
\end{center}
\end{figure}
...
\begin{center}
\includegraphics[width=1\columnwidth]{figures/rates-of-return-ibm-heatmap/rates-of-return-ibm-heatmap.png}
\end{center}
\end{figure}
...
\begin{center}
\includegraphics[width=1\columnwidth]{figures/rates-of-return-ko-heatmap/rates-of-return-ko-heatmap.png}
\end{center}
\end{figure}
...
\begin{center}
\includegraphics[width=1\columnwidth]{figures/rates-of-return-msft-heatmap/rates-of-return-msft-heatmap.png}
\end{center}
\end{figure}
